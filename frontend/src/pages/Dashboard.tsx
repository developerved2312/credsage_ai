    ...
    <div className="space-y-6">
      ...
      <div className="card flex items-start justify-between gap-6">
        ...
        {latestScore && (
          <div className="hidden md:flex flex-col items-end gap-1 shrink-0">
            ...
            <div className="flex justify-between w-40 text-xs text-text-secondary">
              <span>300</span>
              <span>850</span>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <div className="w-8 h-8 rounded bg-green-50 border border-green-200 flex items-center justify-center shrink-0">
          ...
          >
            <div className="w-8 h-8 rounded bg-green-50 border border-green-200 flex items-center justify-center shrink-0">
              <Star size={15} className="text-primary" strokeWidth={1.75} />
            </div>
            <div>
              <p className="text-sm font-medium text-text-primary group-hover:text-primary transition-colors">
          ...
          >
            <div className="w-8 h-8 rounded bg-green-50 border border-green-200 flex items-center justify-center shrink-0">
              <MessageSquare size={15} className="text-primary" strokeWidth={1.75} />
            </div>
            <div>
              <p className="text-sm font-medium text-text-primary group-hover:text-primary transition-colors">
                Ask the AI advisor
      ...
      {analytics?.topPerformers && analytics.topPerformers.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-semibold text-text-primary">Top Performers</h2>
            <Link to="/investment/portfolio" className="text-xs text-primary hover:underline font-medium">
              View portfolio →
            </Link>
          </div>
      ...
      {(!analytics?.topPerformers || analytics.topPerformers.length === 0) && (
        <div className="card flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Briefcase size={18} className="text-text-secondary" strokeWidth={1.5} />
            <div>
          ...
          <Link to="/investment/portfolio" className="btn btn-secondary text-xs">
            Create Portfolio
          </Link>
        </div>
      )}
